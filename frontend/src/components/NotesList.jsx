  ...
  return (
    <nav className="overflow-y-auto flex-1 flex flex-col gap-1">
      {notes.map(note => (
        <motion.button
          key={note.id}
          whileHover={{ scale: 1.01 }}          className={`w-full text-left p-3 rounded-lg transition-all duration-200 ${
            note.id === activeId 
              ? "bg-chatgpt-selected-note text-chatgpt-text-primary shadow-sm" 
        ...
        >          <div className={`line-clamp-1 font-medium text-xs ${
            note.id === activeId ? "text-chatgpt-text-primary" : "text-chatgpt-text-primary"
          }`}>
            {note.title || "Untitled"}
          </div>
          <div className={`text-xs line-clamp-2 mt-1 ${
            note.id === activeId ? "text-chatgpt-text-secondary opacity-80" : "text-chatgpt-text-secondary"
          }`}>
